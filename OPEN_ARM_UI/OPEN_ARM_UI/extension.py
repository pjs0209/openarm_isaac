...
GRIPPER_RAW_LIMIT = {
    "openarm_left_finger_joint1": (0.0, 0.05),
    "openarm_right_finger_joint1": (0.0, 0.05),
}
...
class OpenArmAutoController(omni.ext.IExt):
    def on_startup(self, ext_id):
        ...
        self.preferred_root_path = "/openarm/root_joint"
        self.base_search_path = "/openarm"
    ...
    def _build_ui(self):
        ...
        with self.window.frame:
            with ui.VStack(spacing=8):
                ...
                # Content: 두 컨테이너를 모두 만들고 visible 토글로 탭처럼 보이게 함
    ...
    def _show_tab(self, name: str):
        """탭 에뮬레이션: left/right 컨테이너 visible 토글"""
    ...
    def _build_sliders(self):
        # DOF name -> (handle, current_rad_or_raw)
    ...
    def _add_joint_row(self, name: str, name_map: dict, st: dict):
        ...
        # 범위/표시값 결정
        if grip:
            lo, hi = GRIPPER_RAW_LIMIT.get(name, (0.0, 0.05))
            cur_ui = float(cur_rad)
            unit = ""     # raw
            tgt_ui = cur_ui
            self.targets[dof] = float(cur_rad)   # raw
        else:
            lo, hi = JOINT_LIMIT_DEG.get(name, (-180.0, 180.0))
            cur_ui = math.degrees(float(cur_rad))
            unit = "°"
            tgt_ui = cur_ui
            self.targets[dof] = float(cur_rad)   # rad
    ...
    def _on_zero_position(self):
        ...
        for dof, slider in self.slider_by_dof.items():
            ...
            ui_zero = 0.0
            slider.model.set_value(ui_zero)

            if grip:
                self.targets[dof] = 0.0
            else:
                self.targets[dof] = 0.0  # 0deg -> 0rad
